...
- **HTML** (default) — Self-contained `.html` file with embedded CSS, inline SVG charts, and CSS page/section breaks. Opens in any browser. Maximum control over formatting and styling.
...
**Charts and visuals:**
- Render all charts as inline SVG directly in the HTML per the inline SVG chart standards in `references/slide-design.md` HTML/CSS Rendering section
...
- Are inline SVG charts rendering with correct brand colors and readable labels?
- Does keyboard navigation work (arrow keys scroll between sections)?
- Is the 16:9 aspect ratio preserved — slides do not stretch or compress at common viewport sizes?
...
- **[HTML mode] Are charts inline SVG with brand colors?** No canvas elements, no external charting libraries. SVG elements use the same brand palette as the rest of the deck.
...
- **Slides mode (HTML):** Save the `.html` file to `knowledge/presentations/YYYY-MM-DD-descriptive-slug.html`.
- **Slides mode (PDF):** Save both the `.html` intermediate and the `.pdf` final to `knowledge/presentations/YYYY-MM-DD-descriptive-slug.[html|pdf]`.
- **Slides mode (PPTX):** Save instruction is embedded in the execution steps above — write the `.pptx` file to `knowledge/presentations/YYYY-MM-DD-descriptive-slug.pptx`.
